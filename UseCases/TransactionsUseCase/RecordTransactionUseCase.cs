...
namespace UseCases
{
    public class RecordTransactionUseCase : IRecordTransactionUseCase
    {
        ...
        public void Execute(string cashierName, int productId, int qty, bool isForSell)
        {
            var product = getProductByIdUseCase.Execute(productId);
            if (isForSell == true)
            {
                transactionRepository.Save(cashierName, productId, product.Name, product.Price.Value, product.Quantity.Value, qty, true);
            }
            else
            {
